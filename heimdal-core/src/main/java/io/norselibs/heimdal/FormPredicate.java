...
public class FormPredicate<T> {
    private final T proxyInstance;
    private final QueryWrapper queryWrapper;
    private PredicateNode result;

    FormPredicate(T proxyInstance, QueryWrapper queryWrapper) {
        this.proxyInstance = proxyInstance;
        this.queryWrapper = queryWrapper;
    }

    public <X> FormPredicate<T> eq(Function<T, X> field, X value) {
    ...
    public <X> FormPredicate<T> neq(Function<T, X> field, X value) {
        field.apply(proxyInstance);
        String fieldName = queryWrapper.getCurrentProperty().getToken().camelHump();
    ...
    @SafeVarargs
    public final <X> FormPredicate<T> in(Function<T, X> field, X... values) {
        field.apply(proxyInstance);
        String fieldName = queryWrapper.getCurrentProperty().getToken().camelHump();
